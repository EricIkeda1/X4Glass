...
- Para rodar o servidor, você deve estar na pasta principal do projeto (onde o arquivo ```manage.py``` está localizado). Esse é o diretório que contém a configuração principal do Django e onde o comando ``` python manage.py runserver_plus --cert-file cert.pem --key-file cert.key  ```  deve ser executado.
